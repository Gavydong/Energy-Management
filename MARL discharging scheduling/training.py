...
def act(model,state,epsilon,e,action_size=288):
...
def validation_act(model,state,action_size=288):
...
def main():
    ...
    model_6 = create_model(state_size=state_size,action_size= action_size)
    model_7 = create_model(state_size=state_size,action_size= action_size)
    model_8 = create_model(state_size=state_size,action_size= action_size)
    model_9 = create_model(state_size=state_size,action_size= action_size)
    model_10 = create_model(state_size=state_size,action_size= action_size)
    ...
    for e in range(episode_count + 1):
        ...
        while state_date<datetime.strptime("2018-05-01","%Y-%m-%d"):
            ...
            #print("Start {0} seconds.".format(training_time))
            ...
            action_6=act(model_1,state,epsilon,e)
            action_7=act(model_2,state,epsilon,e)
            action_8=act(model_3,state,epsilon,e)
            action_9=act(model_4,state,epsilon,e)
            action_10=act(model_5,state,epsilon,e)
            discharge_1 = action_1
            discharge_2 = action_2
            discharge_3 = action_3
            discharge_4 = action_4
            discharge_5 = action_5
            discharge_6 = action_6
            discharge_7 = action_7
            discharge_8 = action_8
            discharge_9 = action_9
            discharge_10 = action_10
            ...
            Shaved_value[discharge_6:discharge_6 + discharge_hour * 12] = Shaved_value[
                                                                     discharge_6:discharge_6 + discharge_hour * 12] - capacity / discharge_hour
            Shaved_value[discharge_7:discharge_7 + discharge_hour * 12] = Shaved_value[
                                                                     discharge_7:discharge_7 + discharge_hour * 12] - capacity / discharge_hour
            Shaved_value[discharge_8:discharge_8 + discharge_hour * 12] = Shaved_value[
                                                                     discharge_8:discharge_8 + discharge_hour * 12] - capacity / discharge_hour
            Shaved_value[discharge_9:discharge_9 + discharge_hour * 12] = Shaved_value[
                                                                     discharge_9:discharge_9 + discharge_hour * 12] - capacity / discharge_hour
            Shaved_value[discharge_10:discharge_10 + discharge_hour * 12] = Shaved_value[
                                                                     discharge_10:discharge_10 + discharge_hour * 12] - capacity / discharge_hour
            #reward=0
            ...
            #fitting model
            model_fit(model_1, action_1, state, peak_shave)
            model_fit(model_2, action_2, state, peak_shave)
            model_fit(model_3, action_3, state, peak_shave)
            model_fit(model_4, action_4, state, peak_shave)
            model_fit(model_5, action_5, state, peak_shave)
            model_fit(model_6, action_6, state, peak_shave)
            model_fit(model_7, action_7, state, peak_shave)
            model_fit(model_8, action_8, state, peak_shave)
            model_fit(model_9, action_9, state, peak_shave)
            model_fit(model_10, action_10, state, peak_shave)
...
#    validation_peak_shave_save.to_csv("data/validation_peak_shave.csv", sep=",", index=False)
    if not os.path.exists("10models"):
        os.mkdir("10models")
    model_1.save("10models/model_agent_1")
    model_2.save("10models/model_agent_2")
    model_3.save("10models/model_agent_3")
    model_4.save("10models/model_agent_4")
    model_5.save("10models/model_agent_5")
    model_6.save("10models/model_agent_6")
    model_7.save("10models/model_agent_7")
    model_8.save("10models/model_agent_8")
    model_9.save("10models/model_agent_9")
    model_10.save("10models/model_agent_10")
